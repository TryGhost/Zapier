...
describe('Triggers', function () {
    describe('Member Deleted', function () {
        ...
        it('loads member from fake hook', function () {
            ...
            return appTester(App.triggers.member_deleted.operation.perform, bundle).then(
                ([member]) => {
        ...
        it('loads member from list', function () {
            ...
            return appTester(App.triggers.member_deleted.operation.performList, bundle).then(
                (results) => {
        ...
        it('subscribes to webhook', function () {
            ...
            return appTester(App.triggers.member_deleted.operation.performSubscribe, bundle).then(
                () => {
                    expect(apiMock.isDone()).toBe(true);
                },
            );
        ...
        it('unsubscribes from webhook', function () {
            ...
            apiMock.delete('/ghost/api/admin/webhooks/subscribe-test/').reply(204);

            return appTester(App.triggers.member_deleted.operation.performUnsubscribe, bundle).then(
                () => {
                    expect(apiMock.isDone()).toBe(true);
                },
            );
