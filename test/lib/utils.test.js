    ...
    describe('initAdminApi', function () {
        it('requests unversioned admin paths with an Accept-Version header', function () {
            ...
            return api.site.read().then((site) => {
                ...
                expect(z.requestOptions.url).toEqual(
                    'http://zapier-test.ghost.io/ghost/api/admin/site/',
                );
        ...
        it('prefixes the SDK User-Agent with the Zapier version', function () {
            ...
            return api.site.read().then(() => {
                expect(z.requestOptions.headers['User-Agent']).toMatch(
                    new RegExp(`^Zapier/${packageVersion} GhostAdminSDK/\\d+\\.\\d+\\.\\d+$`),
                );
            });
        });
            ...
            return api.site.read().then(
                ...
                (err) => {
                    ...
                    expect(err.message).toEqual(
                        'Resource not found error, cannot read member. (NotFoundError)',
                    );
                },
            );
        });
        ...
        it('includes the error code in halted validation errors', function () {
            ...
            return api.site.read().then(
                ...
                (err) => {
                    expect(err.name).toEqual('HaltedError');
                    expect(err.message).toEqual(
                        'Validation failed for email (ValidationError: UNIQUE_EMAIL)',
                    );
