...
describe('Authentication', function () {
    ...
    describe('test', function () {
        it('is success with valid api key and Ghost 6.0', function () {
            ...
            apiMock.get('/ghost/api/admin/site/').reply(200, {
                site: { version: '6.0' },
            });

            apiMock.get('/ghost/api/admin/config/').reply(200, {});
        ...
        it('errors with invalid Admin API Key', function () {
            ...
            apiMock.get('/ghost/api/admin/config/').reply(400, {
                errors: [
                    {
        ...
        it('errors when the Admin API is not found', function () {
            ...
            apiMock.get('/ghost/api/admin/site/').reply(404);
            ...
            return appTester(App.authentication.test, bundle).then(
                ...
                (err) => {
                    expect(err.message).toMatch(
                        /^Supplied 'Admin API URL' does not point to a Ghost site with a supported version \(>=6\.0\)/,
                    );
                    expect(nock.pendingMocks().length).toEqual(0);
                },
            );
